...
namespace Aldebaran.DataAccess.Entities
{
    public class WarehouseTransferConfiguration : IEntityTypeConfiguration<WarehouseTransfer>
    {
        public void Configure(EntityTypeBuilder<WarehouseTransfer> builder)
        {
            ...
            builder.HasKey(x => x.WarehouseTransferId).HasName("PK_WARHOUSE_TRANSFER").IsClustered();
            ...
            builder.Property(x => x.OrigenWarehouseId).HasColumnName(@"ORIGEN_WAREHOUSE_ID").HasColumnType("smallint").IsRequired();
            ...
            // Foreign keys
            builder.HasOne(a => a.OrigenWarehouse).WithMany(b => b.OriginWarehouseTransfers).HasForeignKey(c => c.OrigenWarehouseId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_WARHOUSE_TRANSFER_ORIGEN_WAREHOUSE");
            builder.HasOne(a => a.DestinationWarehouse).WithMany(b => b.DestinationWarehouseTransfers).HasForeignKey(c => c.DestinationWarehouseId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_WARHOUSE_TRANSFER_DESTINATION_WAREHOUSE");
            builder.HasOne(a => a.Employee).WithMany(b => b.WarehouseTransfers).HasForeignKey(c => c.EmployeeId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_WARHOUSE_TRANSFER_EMPLOYEE");
            builder.HasOne(a => a.StatusDocumentType).WithMany(b => b.WarehouseTransfers).HasForeignKey(c => c.StatusDocumentTypeId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_WARHOUSE_TRANSFER_STATUS_DOCUMENT_TYPE");
        }
    }
}
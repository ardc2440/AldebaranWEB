...
{
    public class AdjustInventoryFromOrderShipmentCancelled : InventoryManagementBase, IBeforeSaveTrigger<CustomerOrderShipment>
    {
        private readonly AldebaranDbContext _context;

        public AdjustInventoryFromOrderShipmentCancelled(AldebaranDbContext context) : base(context)
        {
        ...
        public async Task BeforeSave(ITriggerContext<CustomerOrderShipment> context, CancellationToken cancellationToken)
        {
            ...
            foreach (var item in context.Entity.CustomerOrderShipmentDetails)
            {
                ...
                await UpdateWorkInProcessQuantityAsync(reference!.ReferenceId, item.DeliveredQuantity, 1, cancellationToken);
            }

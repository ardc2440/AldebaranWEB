...
{
    public partial class CustomerOrderClosesLog
    {
        #region Injections
        [Inject]
        ...
        #region Variables
        protected IEnumerable<ServiceModel.ClosedCustomerOrder> ClosedCustomerOrders;
        protected LocalizedDataGrid<ServiceModel.ClosedCustomerOrder> ClosedCustomerOrdersDataGrid;
        ...
        {
            await Task.Yield();
            (ClosedCustomerOrders, count) = await CustomerOrderService.GetCustomerOrderClosesLogAsync(skip, top, searchKey, ct);
        }

        protected async Task Search(ChangeEventArgs args)
        {
            search = $"{args.Value}";
            await ClosedCustomerOrdersDataGrid.GoToPage(0);
            await GetDataLogAsync(search);
        }

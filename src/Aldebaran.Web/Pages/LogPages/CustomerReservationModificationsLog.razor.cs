...
{
    public partial class CustomerReservationModificationsLog
    {
        #region Injections
        [Inject]
        protected ILogger<CustomerReservationModificationsLog> Logger { get; set; }
        ...
        #region Variables
        protected IEnumerable<ServiceModel.ModifiedCustomerReservation> ModifiedCustomerReservations;
        protected LocalizedDataGrid<ServiceModel.ModifiedCustomerReservation> ModifiedCustomerReservationsDataGrid;
        ...
        {
            await Task.Yield();
            (ModifiedCustomerReservations, count) = await CustomerReservationService.GetCustomerReservationModificationsLogAsync(skip, top, searchKey, ct);
        }

        protected async Task Search(ChangeEventArgs args)
        {
            search = $"{args.Value}";
            await ModifiedCustomerReservationsDataGrid.GoToPage(0);
            await GetDataLogAsync(search);
        }

...
{
    public class EmailNotificationProviderSettingsRepository : IEmailNotificationProviderSettingsRepository
    {
        private readonly AldebaranDbContext _context;
        public EmailNotificationProviderSettingsRepository(AldebaranDbContext context)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context));
        }
        public async Task<EmailNotificationProvider> GetAsync(string subject, CancellationToken ct = default)
        {
            var provider = await _context.NotificationProviderSettings.AsNoTracking().FirstOrDefaultAsync(f => f.Subject == subject, ct);
            if (provider == null)
        ...
        public async Task UpdateAsync(string subject, EmailNotificationProvider provider, CancellationToken ct = default)
        {
            ...
            var currentProvider = await _context.NotificationProviderSettings.FirstOrDefaultAsync(f => f.Subject == subject, ct);

...
BEGIN
	...
	DECLARE @TOTAL_PROCESSED_QUANTITY INT, @TOTAL_DELIVERED_QUANTITY INT, @STATUS_ORDER SMALLINT, @CUSTOMER_ORDER_STATUS_ID SMALLINT 
	...
	BEGIN
		...
		UPDATE c
		   SET c.PROCESSED_QUANTITY = c.PROCESSED_QUANTITY - b.PROCESSED_QUANTITY
		  FROM inserted a 
		...
		IF @TOTAL_PROCESSED_QUANTITY = 0 AND @TOTAL_DELIVERED_QUANTITY = 0  
			SET @STATUS_ORDER = 1
		ELSE
			IF @TOTAL_DELIVERED_QUANTITY = 0 
				SET @STATUS_ORDER = 2

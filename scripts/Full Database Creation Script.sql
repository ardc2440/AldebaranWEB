...
IF OBJECT_ID('DateTimeToStringFormated', 'FN') IS NOT NULL
	DROP FUNCTION DateTimeToStringFormated
GO
...
(
	...
	VISUALIZED_DATE DATETIME NOT NULL CONSTRAINT DF_VISUALIZED_ALARMS_VISUALIZED_DATE DEFAULT GETDATE(),
...
CREATE OR ALTER PROCEDURE [dbo].[SP_GET_REFERENCE_MOVEMENT_REPORT]
	@ReferenceIds VARCHAR(MAX) = '',
...
BEGIN		
	...
	DECLARE @CustomerOrders TABLE 
		( CustomerId INT, 
		  ...
		  OrderDetailItemName VARCHAR(50), 
	...
	DECLARE @ShippingOrders TABLE
		( CustomerOrderDetailId INT, 
		  ...
		  ShipmentDetailItemName VARCHAR(50), 
...
BEGIN 
	...
	SELECT a.PURCHASE_ORDER_ID OrderId, b.STATUS_DOCUMENT_TYPE_NAME StatusDocumentTypeName, a.ORDER_NUMBER OrderNumber, a.CREATION_DATE CreationDate, a.REQUEST_DATE RequestDate, a.EXPECTED_RECEIPT_DATE ExpectedReceiptDate, c.PROVIDER_NAME ProviderName,  
		   ...
		   j.REFERENCE_CODE ReferenceCode, j.REFERENCE_NAME ReferenceName, CASE b.STATUS_ORDER WHEN 2 THEN h.RECEIVED_QUANTITY ELSE h.REQUESTED_QUANTITY END Amount, 
		   k.WEIGHT * CASE b.STATUS_ORDER WHEN 2 THEN h.RECEIVED_QUANTITY ELSE h.REQUESTED_QUANTITY END Weight, 
		   k.VOLUME * CASE b.STATUS_ORDER WHEN 2 THEN h.RECEIVED_QUANTITY ELSE h.REQUESTED_QUANTITY END Volume
	  FROM purchase_orders a
...
CREATE OR ALTER FUNCTION dbo.DateTimeToStringFormated
(
    @Date DATETIME,
	@Format VARCHAR(10)
)
